...
public class EditAvailableVehicleScreen extends LargeView {
	...
	public EditAvailableVehicleScreen(int vehicleId) {
		this.setTitle(this.getTitle() + "Editar veiculo");
	...
	private void populateFiles(List<Archive> archives) {
		for (int i = 0; i < archives.size(); i++) {
			...
			try {
				...
				if (i == 0) {
					vehicleFirstFileChooser.setSelectedFile(tempFile);
					vehicleFirstFileChooserLabel.setText(archive.getFilename());
				} else if (i == 1) {
					vehicleSecondFileChooser.setSelectedFile(tempFile);
					vehicleSecondFileChooserLabel.setText(archive.getFilename());
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
